...
EOF
      ...
      service {
        port = "client"
        address_mode = "driver"
        name = "kafka-zookeeper-standalone-zk1-docker"
        tags = [
          "zookeeper-client-docker"
        ]
        check {
          type = "script"
          name = "status"
...
//        KAFKA_LISTENER_SECURITY_PROTOCOL_MAP = "PLAINTEXT:PLAINTEXT"
        ...
        KAFKA_LISTENERS = "PLAINTEXT://kafka1:9092,PLAINTEXT_HOST://${NOMAD_IP_kafka}:${NOMAD_HOST_PORT_kafka}"
        KAFKA_ADVERTISED_LISTENERS = "PLAINTEXT://kafka1:9092,PLAINTEXT_HOST://${NOMAD_IP_kafka}:${NOMAD_HOST_PORT_kafka}"
      ...
      service {
        port = "kafka"
        address_mode = "driver"
        name = "kafka-zookeeper-standalone-ka1-docker"
        tags = [
          "kafka-docker"
        ]
      }
